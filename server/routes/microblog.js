...
router.get('/history/:room', function (req, res) {
  try {
    microblogCtrl
      .getMicroblogHistory(req.params.room, req.query.limit, req.query.offset)
      .then(
        function (microblog) {
...
router.get('/allmicroblogs', function (req, res) {
  try {
    microblogCtrl.getMicroblogs().then(
      function (microblogs) {
        res.send(microblogs);
      },
      function (err) {
...
router.get('/:room', function (req, res) {
  try {
    microblogCtrl.getMicroblogByRoom(req.params.room).then(
      function (microblog) {
        var clonedMicroblog = microblog.getJSON();
        console.log('clonedMicroblog===', clonedMicroblog);
        microblogCtrl._getMicroblogMeta(microblog).then(
          function (meta) {
...
router.post('/', function (req, res) {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    try {
      microblogCtrl.createMicroblog(data).then(
        function (result) {
...
router.post('/microblogpic/:room', function (req, res) {
  microblogCtrl.updateMicroblogPic(req.params.room, req.body).then(
    function (microblog) {
...
router.post('/upload-image-message', function (req, res) {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    try {
      microblogCtrl.saveImageMessage(data).then(
        function (response) {
          res.send(response);
        },
        function (err) {
...
router.post('/newNotification', function (req, res) {
  console.log('8888888888888', res.body)
  var type = req.body.type
  var data = {}
  if(type === 'friend-request') data = {from: req.body.from, to: req.body.to, name:req.body.name}
  notificationCtrl.addNotification(type, data);
  res.send('Successfully sent invitation to microblog');
});

router.post('/remove-user', function (req, res) {
  microblogCtrl.removeMicroblogUser(req.body.user, req.body.room).then(
    function () {
