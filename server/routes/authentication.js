...
module.exports = function (passport) {
  ...
  router.get('/failure', function (req, res) {
    ...
    var flashObjectProperty = flashObject[Object.keys(flashObject)[0]];
    var failureMessage =
      flashObjectProperty?.length > 0
        ? flashObjectProperty[0]
        : flashObjectProperty;
  ...
  router.post('/resend-email', function (req, res) {
    user.resendEmail(req.body.email).then(
      function () {
        res.send({state: 'success'});
      },
      function (err) {
  ...
  router.post('/activate-account', function (req, res) {
    user.activate(req.body.token).then(
      function (user) {
  ...
  router.post('/restore-password', function (req, res) {
    user.restorePassword(req.body.email).then(
      function (token) {
  ...
  router.post('/new-password', function (req, res) {
    user
      .createNewPassword(req.body.email, req.body.password, req.body.token)
      .then(
        function (user) {
          res.send({
            state: 'success',
            message: 'Successfully reset your password! Logging in...',
          });
        },
        function (err) {
          res.send({state: 'error', message: err});
  ...
  router.get('/me', function (req, res) {
    try {
      user.getUserByEmail(req.session.email).then(
        function (user) {
          res.send(user);
        },
        function (err) {
  ...
  router.get('/user', function (req, res) {
    try {
      user.getRememberMeUser(req.query.key).then(
        function (user) {
          res.send(user);
        },
        function (err) {
