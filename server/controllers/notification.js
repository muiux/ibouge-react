  ...
  addNotification: function (type, data, cb) {
    switch (type) {
      ...
      case 'reply-request':
        ...
        break;
      case 'poster-request':
        posterNotification(type, data, cb);
        break;
    }
  },
};
  ...
  if (userId) {
    User.findOne({_id: userId}, function (err, user) {
      if (user) {
        ...
        newNotification.recipients = chat.users.map(function (item) {
          return item.user_id;
        });
...
function posterNotification(type, data) {
  // var meta = [];
  // meta.push({key: 'from', value: data.from});
...
function inviteFriendToMicroblogNotification(type, data) {
  ...
  if (userId) {
    User.findOne({_id: userId}, function (err, user) {
      var text =
        user.fname + ' ' + user.lname + ' has invited you to join a microblog';
      var meta = [];
      meta.push({key: 'created_by', value: userId});
  ...
  User.updateOne(
    ...
    function (err) {
      if (err) {
        ...
        console.log(
          'A new notification is added to the list of notifications for the user.'
        );
      }
      if (cb) {
