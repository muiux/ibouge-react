...
// import {generateRoomName} from '../../helpers/chat';
...
const Chatroom = () => {
  ...
  useEffect(() => {
    ...
    if(document.getElementById('small-chat-container'))
      document.getElementById('small-chat-container').scrollTop = document.getElementById('small-chat-container').clientHeight
  }, [chatStore]);
  ...
  useEffect(() => {
    ...
    socket.on('loadP2PChatHistory', function (data) {
      ...
      console.log(chat,'00000000000')
      if (chat) {
        loadChatHistory(chat)
          .then((res) => {
            console.log('res on loadp2pchathistory',res);
          })
